...
% 
% 
% X1=X1+0.01*(X0-X1)+0.001*0.01*W1'*inv(W1*X1*X1'*W1')*W1*X1;
% 
% 
...
% for iter=1:1*r
 ...
 A_est=[A_est1];
  ...
  S_est=[S_est1];     
  Xest=A_est1*S_est;    
%err(iter1)=norm(S_est-H0,'fro')/sqrt(mm0*nn0);
 %    X1=inv(W)*W*X1; 
  [ratio(iter1)] = dictdist(W,Omega,0.01);
  [recoveryH(iter1)]=dictdist(S_est',H0',0.01);
  % DET(iter1)=DET1(iter1);

    DET(iter1) = Sparsity_Hoyer(S_est);
     if iter1>1 && DET(iter1)<DET(iter1-1)
         %iter1=iter1-1;
     end
end
