...
DimTar=1; % target subspace dimension
p=n*(n+2); % number of atoms
IterTrain=20; %number of iterations
PursuitType=2; 
AtomUpdateType=3;
ImNames={'barbara'}; %{'lena','barbara','boats','house','peppers256','face_01'};
for j=1:length(ImNames)
    ...
    for k=1:length(SigmaVec)
        ...
        DenoisingError(1)=norm(normcols(Xdn(:,Idx0))-normcols(X(:,Idx0)),'fro')/sqrt(d*length(Idx0));
        CoSparsityLevel(1)=d;
...
%         figure,imagesc(IMAGE); axis image; axis off; colormap(gray(256));
      ...
      %  %Omega=AnalysisKSVD(Xntrain,DimTar,p,IterTrain,PursuitType,OmegaInit,AtomUpdateType,Xtrain);            
       % [A_est, Omega, S_est, ratio, recoveryH, DET]=IVM_QP00(Xntrain,d,OmegaInit,H0);
       [A_est Omeganew S_est ratio recoveryH DET]=IVM_QP1(X,d,OmegaInit,H0);
...
%         title(['Recovered image: PSNR=',num2str(PSNRdn),'[dB]'])
        ...
        h1=figure;
        OmegaDisp=DisplayOmega(Omeganew,h1);
         title(ImNames)
%          N1=40;
%         count=1;
%         IMAGE=zeros(1,1+(n+1)*N1);
