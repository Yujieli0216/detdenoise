...
while iter<=1*M
   ...
   % H=(obj_coef+obj_coef')/2;
    ...
    A=-eye(M);
    b=zeros(M,1);
    lb=pinv(Ai(:,p:q))*(Xj-1e-5);
    ub=pinv(Ai(:,p:q))*(Xj+1e-5);
    
   
    [y,fval,exitflag]=quadprog(-H,f,A,b,Aeq,beq,lb,ub,S(i,:)',optimset('Display','off'));
...
%     Ai=Ai./(ones(m,1)*normcol);
 ...
 %       end
       ...
       % Xest=Xest./(sum(Xest)) ;
        ...
        Cost(iter)=sum(sum((Xest-X).^2))-na*abs(det(S'*S));
